...
class DbTest(unittest.TestCase):
    ...
    def test_purge_old_infected_users_list(self):
        ...
        users = config.REDIS_CLIENT.lrange(yday_key, 0, -1)
        self.assertEqual(len(users), 0)

        users = config.REDIS_CLIENT.lrange(today_key, 0, -1)
    ...
    def test_migrate_all_infected_user_reports(self):
        ...
        for user in [fakeuser1, fakeuser2, fakeuser3]:
            config.REDIS_CLIENT.rpush(config.REDIS_LATEST_INFECTED_USERS_KEY, json.dumps(user))
